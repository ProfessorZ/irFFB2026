...
static float ffbGraphClipPct[FFB_GRAPH_SECONDS] = { 0 };  // clipped frames % per second
            ...
            if (*speed > 2.0f) {
                ...
                if (*speed > 5.0f) {
                  ...
                  // text(L"Main: Starting FFB Force: %d", force);
                    ...
                    {
                        // Require a real lateral-load reading: without LatAccel we can't
                        // confirm we're in the linear region, so skip calibration rather
                        // than risk learning a slip-biased K on SDK variants that lack it.
                    ...
                    if (underSteerFactor > 0.0f) {
                        ...
                        // Act only in the understeer direction. When alpha_f opposes the
                        // steer input the car is over-rotating (yaw exceeds the neutral
                        // reference) — that regime belongs to the oversteer channel, so
                        // zero it here to avoid a negative alpha_f feeding yaw_under and
                        // adding into the oversteer direction (double-counting yaw_over).
                        if (alpha_f * (*steer) < 0.0f) {
                            alpha_f = 0.0f;
                        }
...
LRESULT CALLBACK ffbGraphProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_PAINT: {
        ...
        if (plotW > 8 && plotH > 8) {
            ...
            if (count > 0) {
                ...
                for (int x = 0; x < plotW; x++) {
                    ...
                    bool clp = ffbGraphClipPct[idx] > 0.0f;
                    if (clp != clipActive) {
        ...
        // Title + live readouts in the top strip.
        {
            ...
            // Average the per-second frame-accurate clip fractions over the window.
            float clipSum = 0.0f;
            for (int i = 0; i < count; i++) {
                int idx = (ffbGraphHead - 1 - i + FFB_GRAPH_SECONDS) % FFB_GRAPH_SECONDS;
                clipSum += ffbGraphClipPct[idx];
            }
            float clipWinPct = count > 0 ? clipSum / (float)count : 0.0f;
    ...
    switch (message) {
        ...
        case WM_TIMER: {
            if (wParam == FFB_GRAPH_TIMER_ID) {
                ...
                float clipPct = frames > 0 ? (float)clips * 100.0f / (float)frames : 0.0f;

                ffbGraphPct[ffbGraphHead]     = pct;
                ffbGraphClipPct[ffbGraphHead] = clipPct;
